...
import electron, { ipcMain as ipcMain$1, safeStorage, app as app$1, BrowserWindow } from "electron";
...
function createWindow() {
  win = new BrowserWindow({
    ...
    frame: false
    // 👈 если хочешь полностью убрать заголовок и кнопки
  });
...
async function exchangeCodeForTokens(code2) {
  var _a;
  const data = {
    code: code2,
  ...
  try {
    ...
    if (axios.isAxiosError(error2)) {
      const errorData = (_a = error2.response) == null ? void 0 : _a.data;
      console.error("OAuth error:", errorData);
    }
    throw error2;
  }
}
ipcMain$1.handle("google:exchange-code", async (_, code2) => {
  try {
    const tokens = await exchangeCodeForTokens(code2);
    return tokens;
  } catch (err) {
    console.error(err);
    throw err;
